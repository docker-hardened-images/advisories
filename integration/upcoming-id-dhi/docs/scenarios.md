...
| `affected`, no fixed version | [../examples/scenarios/debian-affected-no-fixed](../examples/scenarios/debian-affected-no-fixed/) | A generated DHI OSV record with no fixed event creates an active finding, and VEX `affected` supplies context. This static fixture is grounded in a pinned public production advisory snapshot for `CVE-2025-12781`; it does not claim a mutable source image. |
