...
| Debian `ID=dhi` OS package (not_affected) | [../examples/e2e-debian](../examples/e2e-debian/README.md) | A DHI Debian package assessed not_affected produces no finding because generated DHI OSV excludes the package/version; the not_affected VEX is optional exact-product context. |
| `affected`, no fixed version | [../examples/scenarios/debian-affected-no-fixed](../examples/scenarios/debian-affected-no-fixed/) | A generated DHI OSV record with no fixed event creates an active finding, and VEX `affected` supplies context. This static fixture is grounded in pinned Python definitions and advisory snapshots for `CVE-2025-12781`; it does not claim a mutable source image. |
