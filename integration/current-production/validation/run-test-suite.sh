...
validate_selected_levels() {
  if [[ ! "$SELECTED_LEVELS" =~ ^[1-4](,[1-4])*$ ]]; then
    echo "error: --level must be a comma-separated list containing only 1, 2, 3, or 4" >&2
    return 1
  fi
}
...
validate_selected_levels || exit 1
require_cmd docker
require_cmd jq
...
expected_tests=0
if is_level_selected 1; then expected_tests=$((expected_tests + 4)); fi
if is_level_selected 2; then expected_tests=$((expected_tests + 3)); fi
if is_level_selected 3; then expected_tests=$((expected_tests + 3)); fi
if is_level_selected 4; then expected_tests=$((expected_tests + 5)); fi

total_pass=$((level1_pass + level2_pass + level3_pass + level4_pass))
total_fail=$((level1_fail + level2_fail + level3_fail + level4_fail))
executed_tests=$((total_pass + total_fail))

overall="PASS"
if [[ "$total_fail" -gt 0 || "$executed_tests" -ne "$expected_tests" ]]; then
  overall="FAIL"
fi

certification_eligible="false"
if [[
  "$level1_pass" -eq 4 && "$level1_fail" -eq 0 &&
  "$level2_pass" -eq 3 && "$level2_fail" -eq 0 &&
  "$level3_pass" -ge 2
]]; then
...
report_json="$(
  jq -n \
    ...
    --arg overall "$overall" \
    --arg selected_levels "$SELECTED_LEVELS" \
    --argjson expected_tests "$expected_tests" \
...
echo "  Executed -> $executed_tests/$expected_tests"
